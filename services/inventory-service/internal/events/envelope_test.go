...
func TestNewEnvelope(t *testing.T) {
	payload := json.RawMessage(`{"stock":"low"}`)
	corrID := "inv-corr-id-456"
	...
	if env.SourceService != "inventory-service" {
		t.Errorf("SourceService = %q, want %q", env.SourceService, "inventory-service")
	}
